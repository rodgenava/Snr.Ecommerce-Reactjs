            ...
            <table>
                ...
                {items.length > 0 ? items.map((item, index) => {
                    if(index + 1 !== items.length){
                        return (<tr key={`${item.store}${item.sku}${item.dateChanged}`}>
                                    <td>{item.store}</td>
                                    <td>{item.sku}</td>
                        ...
                        return (<tr ref={lastElement} 
                                    key={`${item.store}${item.sku}${item.dateChanged}`}>
                                    <td>{item.store}</td>
                                    <td>{item.sku}</td>
                                    <td>{item.description}</td>
                                    <td className='text-right'>{formatWithThousandSeparator(item.currentPrice.toFixed(2))}</td>
                    ...
                    }) : error === null && !isBusy && <tr><td colSpan='6' className='text-center'>No results</td></tr> }
                { error && !isBusy && <tr><td colSpan='6' className='text-center'>An error occurred:(</td></tr> }
                { !isBusy && !hasMore && items.length !== 0 && <tr><td colSpan='6' className='text-center'>End of results.</td></tr> }
                { isBusy && (
