...
function PromotionsUpdate() {
      ...
      const uploadSelectedFile = async () =>{
        ...
        try {
          ...
          const response = await axios({
            method: 'post',
            url: 'api/pricebook4/promotions/update',
            data: formData,
  ...
  const downloadTemplate = function(event){
    ...
    downloadsDispatcher({
      type:'NEW',
      newItem: {
        name: 'Pricebook 4 Promotions',
        saveas: 'Pricebook 4 Promotions.xlsx',
        url: `${ApiConstants.baseUrl}api/pricebook4/promotions/template`
      }
    });
  ...
  return (
    <section>
        ...
        <p className='text-slate-500'>Select update file for S&R Online Store promotions</p>
        <div className='mt-2'>
          ...
          {/* <a className='inline-block m-3 cursor-pointer text-blue-500' href={`${ApiConstants.baseUrl}api/pricebook4/promotions/template`} target='_blank'>I don't have a template</a> */}
