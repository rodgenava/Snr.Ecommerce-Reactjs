...
function ProductsUpdate() {
      ...
      const uploadSelectedFile = async () =>{
        ...
        try {
          ...
          const response = await axios({
            method: 'post',
            url: 'api/pricebook4/skuconfiguration/update',
            data: formData,
  ...
  const downloadTemplate = function(event){
    ...
    downloadsDispatcher({
      type:'NEW',
      newItem: {
        name: 'Pricebook 4 Template',
        saveas: 'Pricebook 4 Template.xlsx',
        url: `${ApiConstants.baseUrl}api/pricebook4/skuConfiguration/template`
      }
    });
  ...
  return (
    <section>
        ...
        <p className='text-slate-500'>Select Sku update file for Lazada and Shopee products</p>
        <div className='mt-2'>
          ...
          {/* <a className='inline-block m-3 cursor-pointer text-blue-500' href={`${ApiConstants.baseUrl}api/pricebook4/skuConfiguration/template`} target='_blank'>I don't have a template</a> */}
