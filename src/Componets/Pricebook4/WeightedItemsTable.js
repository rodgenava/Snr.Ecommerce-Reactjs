...
import useWeightedItemsDataSource from './useWeightedItemsDataSource';
...
function WeightedItemsTable() {

    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(50);
    const [searchTerm, setSearchTerm] = useState('');
    const { error, isBusy, items, hasMore } = useWeightedItemsDataSource(currentPage, pageSize, searchTerm);
  ...
  return (
    <section>
        <h5 className='font-semibold mb-5'>Weighted Skus</h5>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th className='font-medium text-right'>Average Weight</th>
                    </tr>
                </thead>
                <tbody>
                {items.length > 0 ? items.map((item, index) => {
                        if(index + 1 !== items.length){
                            return <tr key={item.sku}>
                                        ...
                                        <td className='text-right'>{item.averageWeight.toFixed(2)}</td>
                                    </tr>;
                        }else{
                ...
                { !isBusy && !hasMore && items.length !== 0 && <tr><td colSpan='5' className='text-center'>End of results.</td></tr> }
                { isBusy && (
...
export default WeightedItemsTable;
