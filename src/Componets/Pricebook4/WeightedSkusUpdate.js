...
function WeightedSkusUpdate() {
      ...
      const uploadSelectedFile = async () =>{
        ...
        try {
          ...
          const response = await axios({
            method: 'post',
            url: 'api/pricebook4/weighteditems/update',
            data: formData,
  ...
  const downloadTemplate = function(event){
    ...
    downloadsDispatcher({
      type:'NEW',
      newItem: {
        name: 'Pricebook 4 Weighted Items',
        saveas: 'Pricebook 4 Weighted Items.xlsx',
        url: `${ApiConstants.baseUrl}api/pricebook4/weighteditems/template`
      }
    });
  ...
  return (
    <section>
        <h5 className='font-semibold mb-5'>Update Weighted Items</h5>
        <p className='text-slate-500'>Select update file for Weighted Items</p>
        <div className='mt-2'>
          ...
          {/* <a className='inline-block m-3 cursor-pointer text-blue-500' href={`${ApiConstants.baseUrl}api/pricebook4/weighteditems/template`} target='_blank'>I don't have a template</a> */}
...
export default WeightedSkusUpdate;
