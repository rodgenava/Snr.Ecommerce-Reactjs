...
function PromotionsUpdate() {
      ...
      const uploadSelectedFile = async () =>{
        ...
        try {
          ...
          const response = await axios({
            method: 'post',
            url: 'api/pricebook3/promotions/update',
            data: formData,
  ...
  const downloadTemplate = function(event){
    ...
    downloadsDispatcher({
      type:'NEW',
      newItem: {
        name: 'Pricebook 3 Promotions',
        saveas: 'Pricebook 3 Promotions.xlsx',
        url: `${ApiConstants.baseUrl}api/pricebook3/promotions/template`
      }
    });
  ...
  return (
    <section>
        ...
        <p className='text-slate-500'>Select update file for Lazada promotions</p>
        <div className='mt-2'>
          ...
          {/* <a className='inline-block m-3 cursor-pointer text-blue-500' href={`${ApiConstants.baseUrl}api/pricebook3/promotions/template`} target='_blank'>I don't have a template</a> */}
